...
async function seedDB() {
  ...
  const travel_imgs = [
    ...
    'https://mypropboutique.com/wp-content/uploads/2017/09/Blue-Large-Vintage-Suitcase.jpg',
  ...
  const totes_imgs = [
    ...
    'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnma79e_iDAi_i2LY_5iX9gcxKR-L1OI15Iw&usqp=CAU',
  ...
  const addProduct = async (category, title, image) => {
  ...
  // Backpacks
  await addProduct('Backpacks', backpacks_titles, backpacks_imgs);

  // Travel Bags
  await addProduct('Travel', travel_titles, travel_imgs);

  // Briefcases
  await addProduct('Briefcases', briefcases_titles, briefcases_imgs);

  // Mini Bags
  await addProduct('Mini Bags', miniBags_titles, miniBags_imgs);

  //Large Handbags
  await addProduct('Large Handbags', largeHandbags_titles, largeHandbags_imgs);

  // Purses
  await addProduct('Purses', purses_titles, purses_imgs);

  // Totes
  await addProduct('Totes', totes_titles, totes_imgs);
